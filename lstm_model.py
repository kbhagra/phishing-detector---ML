...
class FeatureChunkingLSTM(nn.Module):
    """
    Feature-Chunked LSTM for Phishing Detection.
    Processes tabular features in chunks (e.g., 6 timesteps, 8 features per step).
    """
    def __init__(self, input_size=8, hidden_dim=64, num_layers=2, dropout_rate=0.2):
        super(FeatureChunkingLSTM, self).__init__()
        self.lstm = nn.LSTM(
...
def train_lstm_model(epochs=20, batch_size=128, learning_rate=0.002, weight_decay=1e-4):
    ...
    # split 2: split val from train
    val_ratio = 0.25 # 25% of 80% is 20% of the overall data
    ...
    print(f"  - Test set       : {X_test.shape[0]} samples (Locked)")
    ...
    # config
    timesteps = 6
    input_size_per_step = 8 

    # reshape data
    X_tr_seq = torch.tensor(X_train_scaled, dtype=torch.float32).reshape(-1, timesteps, input_size_per_step)
    y_tr_t = torch.tensor(y_train.values, dtype=torch.float32).unsqueeze(1)
    
    X_val_seq = torch.tensor(X_val_scaled, dtype=torch.float32).reshape(-1, timesteps, input_size_per_step)
    y_val_t = torch.tensor(y_val.values, dtype=torch.float32).unsqueeze(1)
    
    X_te_seq = torch.tensor(X_test_scaled, dtype=torch.float32).reshape(-1, timesteps, input_size_per_step)
    ...
    model = FeatureChunkingLSTM(input_size=input_size_per_step, hidden_dim=64, num_layers=2)
    criterion = nn.BCELoss()
    optimizer = optim.Adam(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=3)
    
    print(f"\n--- Feature-Chunked LSTM Architecture ---")
    print(model)
    print(f"\nTraining Chunked LSTM over {epochs} epochs...")
    ...
    for epoch in range(1, epochs + 1):
        ...
        model.eval()
        with torch.no_grad():
            va_outputs = model(X_val_seq)
            va_loss = criterion(va_outputs, y_val_t).item()
            va_preds = (va_outputs.numpy() >= 0.5).astype(int)
            va_acc = accuracy_score(y_val, va_preds) * 100 
    ...
    axes[1].set_title('LSTM: Accuracy over Epochs', fontsize=12, fontweight='bold')
    ...
    plt.title('Chunked LSTM Model: Confusion Matrix', fontsize=12, fontweight='bold')
    plt.colorbar()
    ...
    # save trained model and associated scaler
    models_dir = "models"
